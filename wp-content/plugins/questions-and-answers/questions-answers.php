<?php
/*
Plugin Name: Q & A for WP Accessibility Day
Plugin URI: https://github.com/joedolson/accessible-ajax-comments
Description: Q & A driven by comments. Based on https://github.com/joedolson/accessible-ajax-comments
...
Version: 1.0.0
*/
/*  Copyright 2020  Joe Dolson (email : joe@joedolson.com)
...
add_action( 'wp_enqueue_scripts', 'qa_enqueue_scripts' );
/**
 * Enqueue scripts to handle AJAX comments. Localize script to pass translated responses.
 * Enqueue comment-reply scripts.
 */
function qa_enqueue_scripts() {
	if ( is_singular() && comments_open() ) {
		wp_enqueue_style( 'qa.style', plugins_url( "/css/qa.css", __FILE__ ) );
		wp_enqueue_script( 'qa.comments', plugins_url( "/js/comments.js", __FILE__ ), array('jquery'), '1.1.0', true );
		$comment_i18n = array( 
		...
		wp_localize_script( 'qa.comments', 'qa', $comment_i18n );
	}
}

add_action( 'comment_post', 'qa_ajax_comments', 20, 2 );
/**
 ...
 */
function qa_ajax_comments( $comment_ID, $comment_status ) {
